...
Content: every planting window is checked against at least two independent published sources (different institutions; all UW–Madison pages count as one), and the check is published as the cross-check table on `sources.html`. Keep that standard for every change: when a date moves or a plant is added, update its row on the sources page in the same commit (the test suite fails if the page and the tool disagree about a window or a plant's sources). Deploy `sources.html` next to `index.html` — the tool links to it from the footer and from every plant guide. The Marathon County Master Gardener Volunteers have said they will suggest plants rather than review dates, so credit them for plant suggestions only, in wording they approve, and never describe the dates or tips as reviewed or endorsed by them or by Extension.
...
**Adding a plant.** Add an entry to `PLANTS` in `index.html`: dates are `[month, day]` pairs within March–October, and `src` lists the institutions whose guidance supports the plant's windows (at least two; keys are in `SOURCE_NAMES`). Find each window in at least two independent Extension sources and convert frost-relative advice to the tool's ~May 15 / ~Oct 1 dates, or to the source's own frost reference when it names one (Purdue's calendar, for example, counts from a one-in-ten frost date, May 21 and Sept 21 for Wausau). Add the plant's row to the cross-check table on `sources.html` (`<tr id="check-ID" data-plant="ID" data-src="…">`; the test prints the expected window text if it's off) and any new documents to the source list under it. Draw its illustration in `ART` in the shared palette, or it falls back to a generic sprout with a console warning. Then run the test suite. The printable suggestion sheet, `docs/plant-list.html` (and its PDF), is a dated snapshot for the Master Gardeners' membership meeting; regenerate it only if another round of suggestions is planned.
