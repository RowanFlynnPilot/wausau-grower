...
**Ask a Gardener** — a sponsorable question desk: readers submit gardening questions (every plant guide links to it pre-filled), the sponsoring organization answers them, and the best run in the Ask a Master Gardener column. Sponsor lockup per WPR conventions, `?demo` sales preview, three delivery modes (endpoint, mail app, prototype). Offer sheet in [docs/ask-a-gardener.md](docs/ask-a-gardener.md).
